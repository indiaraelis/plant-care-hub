...
function Dashboard() {
  ...
  // Show the Bell coachmark once: when user has plants and hasn't enabled notifications yet
  useEffect(() => {
    if (
      plants.length > 0 &&
      !enabled &&
      ...
      !localStorage.getItem('pch_bell_hint_shown')
    ) {
      setShowBellHint(true);
    }
  }, [plants.length, enabled, permission]);

  const dismissBellHint = () => {
    localStorage.setItem('pch_bell_hint_shown', 'true');
